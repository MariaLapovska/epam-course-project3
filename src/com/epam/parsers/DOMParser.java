        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (node.getNodeName().equals(SMS_PRICE)) {
                    plan.setSmsPrice(Integer.valueOf(node.getFirstChild().getNodeValue()));
                }
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                if (node.getNodeName().equals(INSIDE_CALLS)) {
                    callPrices.setInsideNetCalls(Integer.valueOf(node.getFirstChild().getNodeValue()));
                }
                if (node.getNodeName().equals(OUTSIDE_CALLS)) {
                    callPrices.setOutsideNetCalls(Integer.valueOf(node.getFirstChild().getNodeValue()));
                }
                if (node.getNodeName().equals(STATIONARY_CALLS)) {
                    callPrices.setStationaryPhoneCalls(Integer.valueOf(node.getFirstChild().getNodeValue()));
                }
            }
        }
